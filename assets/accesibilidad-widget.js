(function () {
  ...
  <div id="sabia-widget-root">
    ...
    <div id="sabia-panel">
      ...
      <div class="s-sliders">
        ...
        <div class="s-srow">
          <span class="s-slabel">Tamaño de fuente</span>
        ...
        <div class="s-srow">
          <span class="s-slabel">Altura de la línea</span>
  ...
  const panel = document.getElementById('sabia-panel');
  document.getElementById('sabia-btn').addEventListener('click', () => panel.classList.toggle('open'));
  document.getElementById('s-close').addEventListener('click', () => panel.classList.remove('open'));

  const filterGroup = ['invert','mono','dark','light','lowsat','highsat'];
  const toggleGroup = ['links','titles','reader','readmode'];

  document.querySelectorAll('.s-opt').forEach(btn => {
    btn.addEventListener('click', () => {
      const a = btn.dataset.a;
      if (filterGroup.includes(a)) {
        const wasOn = btn.classList.contains('active');
        filterGroup.forEach(k => {
          document.querySelector(`[data-a="${k}"]`).classList.remove('active');
          document.body.classList.remove(`sabia-${k}`);
        });
        if (!wasOn) {
          btn.classList.add('active');
          document.body.classList.add(`sabia-${a}`);
        }
      } else {
        const isOn = btn.classList.toggle('active');
        document.body.classList.toggle(`sabia-${a}`, isOn);
        if (a === 'reader') {
          if (isOn) {
            const u = new SpeechSynthesisUtterance(document.body.innerText.substring(0,500));
            u.lang = 'es-MX';
  ...
  const state = { zoom:100, font:100, line:100, spacing:100 };
  function applySliders() {
    document.documentElement.style.zoom          = state.zoom / 100;
    document.documentElement.style.fontSize      = state.font + '%';
    document.documentElement.style.lineHeight    = (state.line / 100 * 1.6).toFixed(2);
    document.documentElement.style.letterSpacing = ((state.spacing - 100) * 0.025) + 'px';
    ['zoom','font','line','spacing'].forEach(k => {
      document.getElementById(`v-${k}`).textContent = state[k] + '%';
      document.getElementById(`f-${k}`).style.width = Math.max(4, ((state[k]-50)/100)*100) + '%';
    });
  }
  document.querySelectorAll('.s-cb').forEach(b => {
    b.addEventListener('click', () => {
      const k = b.dataset.s, d = parseInt(b.dataset.d);
      state[k] = Math.min(200, Math.max(50, state[k] + d * 10));
      applySliders();
    });
  });
  ...
  document.getElementById('s-reset').addEventListener('click', () => {
    ...
    [...filterGroup,...toggleGroup].forEach(k => document.body.classList.remove(`sabia-${k}`));
    window.speechSynthesis.cancel();
    state.zoom = state.font = state.line = state.spacing = 100;
    ['zoom','fontSize','lineHeight','letterSpacing'].forEach(p => document.documentElement.style[p] = '');
    applySliders();
  });
